...
public class Deployer implements Closeable {
  ...
  // private Map<String, Session> sessionMap = new ConcurrentHashMap<String,
  // Session>();
  ...
  public Deployer() {
    // File file = new
    // File(getClass().getClassLoader().getResource("example.txt").getFile());
  }
  ...
  public boolean deploy(EC2 ec2) {
    ...
    try {
      ...
      ec2.threadName = Thread.currentThread().getName();
  ...
  private String getPrivateKeyAbsolutePath() {
    ...
    try {
      ...
      file = new File(classLoader.getResource(privateKey).getFile());
  ...
  private boolean startAndDeployOnGrid(Session session, EC2 ec2) {
    ...
    return this.executeCommand(session, Command.from(command, ec2.getLogTag()));
  }
